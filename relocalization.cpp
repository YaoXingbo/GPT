// 4.负责加载先验地图，通过和先验地图的匹配实现重定位。

// 重定位与slam建图的区别在于slam建图是在对周围环境未知的情况下发生的，是探索地图的过程，
// 而重定位是在已有地图信息的基础上，重新确立自己在地图中的位置的过程。
// 这一部分和mapOptmization.cpp的区别在于前者是和全局性更高的整体先验地图匹配，
// 而后者只是和历史帧地图进行匹配，相对而言缺少部分信息，因此理论上重定位的精度要高于mapOptmization部分。
// 理论上重定位可以通过提供初始位姿，从先验地图上的任意位置开始运行，
// 但本代码没有提供该接口，因此需要从建图的起点开始运行，允许少许的位姿偏差。
#include <ros/ros.h>
#include <math.h>
#include <time.h>
#include <stdio.h>
				     ...
				     slam_algorithm::srv_relocalization::Response &res)  //edit
{
    ...
    //surf_bfile = "/home/robot/e_ws/map/" + req.str_map_name + "_surf.map";
    //corner_bfile = "/home/robot/e_ws/map/" + req.str_map_name + "_corner.map";
...
int main(int argc, char** argv)
{
	...
	ros::Publisher pubOdomAftMapped = nh.advertise<nav_msgs::Odometry> ("/final_odom", 5);
